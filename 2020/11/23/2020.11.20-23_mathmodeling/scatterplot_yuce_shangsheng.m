function judge=scatterplot_yuceshuaijian_sin(height,A,w)
...
while Figure_num < time && Flag_Track && Blue_UAV_Location(end,1)<1.02*10000
	...
	factor1=A;
	factor2=w;
    if Blue_UAV_Location(end,2)<height && flag
		Blue_UAV_Location_new_x = Blue_UAV_Location_temp(1) + step_length * speed_blue_UAV * cos(pi/3);
		Blue_UAV_Location_new_y = Blue_UAV_Location_temp(2) + step_length * speed_blue_UAV * sin(pi/3);
	else
		flag=0;
		Blue_UAV_Location_new_x = Blue_UAV_Location_temp(1) + speed_blue_UAV * step_length ;
		Blue_UAV_Location_new_y = height + factor1 * sin( Blue_UAV_Location_new_x / factor2 );
	end
